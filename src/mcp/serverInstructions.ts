...
export function buildServerInstructions(sendRule: boolean): string {
  return instructionLines(sendRule).join("\n");
}

/** The instructions with the send rule off: today's default. */
export const LETTER_IRL_SERVER_INSTRUCTIONS = buildServerInstructions(false);

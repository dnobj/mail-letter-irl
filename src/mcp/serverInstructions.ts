export const LETTER_IRL_SERVER_INSTRUCTIONS = [
  ...
  "Letter IRL does not generate images. For any request to generate, create, draw, or make an image - even one addressed to Letter IRL by name - generate it immediately with ChatGPT's built-in image generation (image_gen); it is always available, including in Letter IRL conversations, and its images can be attached to Letter IRL previews directly. Never pause to explain that Letter IRL cannot generate images; just generate the image, then offer to use it for a postcard or letter.",

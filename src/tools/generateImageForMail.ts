...
import { openImage, runImageDecode } from "../services/imageService.js";
...
async function createPreview(base64Data: string, actorId: string): Promise<string> {
  const buffer = Buffer.from(base64Data, "base64");
  // The generated bytes come from the image provider, not the customer, but
  // they are opened under the same pixel ceiling and decode gate as every
  // other image, under the same account share: one memory bound for the
  // process, not one per source.
  const preview = await runImageDecode(
    () =>
      openImage(buffer)
...
async function handler(
  ...
  try {
    ...
    const previewBase64 = await createPreview(result.base64Data, userId);

...
- ChatGPT web answered a refused call with a rejection on 2026-09-23 (GIFT-01 step 9, a gift letter send refused by the daily cap, #434). Its message wraps the result's text in a Python-style repr: `Error code: INVALID_ARGUMENT; Error: RuntimeException: Error calling MCP tool: [TextContent(type='text', text='This account has reached its daily limit of 3 items. Please try again tomorrow.', annotations=None, meta=None)]`. The letter, postcard and pack cards' `readableError()` shows only the first `text=` sentence and shows any other message as it came. `includes` matching still sees the whole message. The sentence is a Python string literal: double-quoted when it holds an apostrophe and no double quote, otherwise single-quoted with each apostrophe escaped (`\'`), with `\n`, `\t` and `\r` escaped and `\x`, `\u` or `\U` hex for a character that does not print. The helper reads all of these.

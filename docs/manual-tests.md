...
PR-specific cases are authored here in the implementation PR. Run them against a local or isolated
preview before merge when available. Shared Railway DEV deploys from `dev`, so shared-DEV execution
occurs after merge and deployment; keep the linked issue in validation until that evidence passes.
Unqualified production URLs in the smoke suite are production-only and require explicit owner approval;
for DEV runs, record and use the exact deployed DEV endpoints instead.

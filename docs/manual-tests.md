...
- [ ] Force PostGrid 429 and 5xx responses and confirm bounded retries reuse the exact Idempotency-Key. Force a timeout/connection loss and confirm one submission attempt becomes held/ambiguous. Exhaust definite rejections and confirm an audited retry can restore JIT fulfillment only before refund starts; cross-account/replayed/changed requests fail closed.

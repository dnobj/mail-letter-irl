...
- Remote image URLs are untrusted input. Image processing must require HTTPS, block localhost/private/link-local/reserved IP ranges, validate DNS results before fetch, limit redirects, apply one deadline to the whole transfer including the body, and enforce download size caps even when `Content-Length` is missing. The bytes are untrusted too: every image is opened under a 50-megapixel ceiling that sharp enforces on the header read, the format is checked from the bytes rather than the Content-Type header, each original is decoded once, and decodes and downloads run through in-process concurrency gates so one account's request allowance cannot become an out-of-memory kill (`docs/image-support.md`, Resource limits).

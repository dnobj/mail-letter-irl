...
> 8. When a user asks to generate, create, draw, or make an image, use ChatGPT's built-in image generation; its images can be attached to Letter IRL postcards and letters directly. Call `generate_image_fallback` only when built-in generation is unavailable, blocked, or its image cannot be handed to Letter IRL, or when the user explicitly asks Letter IRL itself to generate the image. After any generation, offer to use the image for a postcard or letter.

...
> 8. When Letter IRL is selected and a user asks to generate, create, draw, or make an image, prefer ChatGPT's native image generation if available. If native image generation is unavailable, blocked, or fails, call `generate_image` as the fallback instead of saying image generation is unavailable. This is appropriate even if the user has not yet provided mailing details; after generation, offer to use the image for a postcard or letter.
